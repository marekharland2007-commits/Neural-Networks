...
def load_data(
    ...
    train_transform = transforms.Compose([
        ...
        # transforms.RandomErasing(p=0.6, scale=(0.02, 0.33))  # Random erasing
    ])
    ...
    # Load test set
    fulltestset = torchvision.datasets.CIFAR100(
        root='./data', 
        train=False, 
        download=download, 
...
def training(model,train_loader, valid_loader, device, epochs=50, warmup_epochs=10, learning_rate=0.001, weightDecay=1e-4, momentum=0.9, mixup_prob=0):
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            ...
            #Ensure that mixup/cutmix is only applied during the first 90% of training to allow the model to fine-tune on real data towards the end
            if epoch < 0.9*epochs:
            ...
            if labels.dim() == 2:  # One-hot (from CutMix/MixUp)
...
if __name__ == "__main__":
    ...
    model = ResNet18(num_classes=100, dropout=0.4).to(device)
    ...
    best_model_state = training(model, train_loader, valid_loader, device, epochs=epochs, warmup_epochs=warmup_epochs, learning_rate=lr, weightDecay=weightDecay, momentum=momentum, mixup_prob=mixup_prob)
    t1 = time.time()
    ...
    with open("trainingLog.txt", "w") as f:
        ...
        print(f"Mixup Probability: {mixup_prob}", file = f)
    ...
    torch.save(model.state_dict(), "CIFAR100Model.pth")
    print("Saved model to CIFAR100Model.pth")

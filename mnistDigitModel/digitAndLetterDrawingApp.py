...
class DigitApp:
    ...
    def preprocess(self):
        ...
        canvas = ImageOps.invert(canvas)  # Make digit black on white if needed

        arr = np.array(canvas).astype(np.float32)
        arr /= 255.0               # now in [0, 1]
        arr = (arr - 0.5) / 0.5    # now in [-1, 1], same as Normalize((0.5,), (0.5,))
    ...
    def predict_digit(self):
        ...
        processed_img.save("processed_28x28.png")
        ...
        lines = [f"{labels[i]}: {probs[i]*100:5.2f}%" for i in top3_idx]
